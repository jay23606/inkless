...
(() => {
  ...
  function showInvitationLinks(invitations) {
    ...
    for (const invite of invitations) {
      ...
      const button = document.createElement("button"); button.type = "button"; button.textContent = "Copy private link";
      button.addEventListener("click", async () => { await navigator.clipboard.writeText(invite.url); toast("Private link copied"); });
      actions.append(button); row.append(label, actions); list.append(row);
    }
